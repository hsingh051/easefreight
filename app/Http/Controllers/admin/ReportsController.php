    ...
    public function index($search_id=NULL){
        if(@$search_id){
            $data = DB::table('searches')
                    ...
                    ->select('payment.*','quotes.*','searches.*','reports.*')
                    ->first();
            //dd($data);
    ...
    public function addreports(Request $request){
        ...
        if(@$post['report_id']){
            ...
            if(@$ff_id){
                ...
                if($this->locale == "es"){
                    return Redirect::to("/es/admin/reports/".$post['search_id'])->with('success',$this->freightforwarderMsg['add']);
                }else{
                    return Redirect::to("/admin/reports/".$post['search_id'])->with('success',$this->freightforwarderMsg['add']);
                }
            } 
        }
        //dd("gfhfgh");
