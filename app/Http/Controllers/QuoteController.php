...
class QuoteController extends Controller
{
    ...
    public function getBooking(Request $request,$search_id){
        ...
        if(@$searches){
            ...
            $data['ff'] = DB::table('users')->select('users.name')->where('users.id','=',$searches->ff_id)->first();
            $data['additional_info'] = DB::table('additional_info')->select('additional_info.*')
                    ->where('additional_info.search_id','=',$data['search_id'])->orderby('additional_info_id','desc')->first();
            if($data['containers']->servicetype == "airfreight"){
    ...
    public function getpayment(Request $request,$search_id){
        ...
        if(@$search_id){
            $data['quote'] = DB::table('quotes')->select('quotes.search_id','quotes.quote_id','quotes.advance','quotes.pending_amount','quotes.grand_total','quotes.advance_payment_document')->where('search_id','=',$search_id)->first();
...
//dd($matches);exit();
        ...
        if($post['status'] == "Aprobado"){
            ...
            DB::table('booking_no')->where('booking_no_id','1')->update($data);
            ...
            $datas['subject'] = "Payment Recived";
            $datas['html'] = "15% Advance payment scuccessfully paid by user. Please <a href='".newurl('/admin/quote/details/'.$quote['search_id'])."'>click here</a> to upload final payment document.";
...
//dd($matches);exit();
    ...
    public function MyOrders(){
        $data = DB::table('searches')
            ->select('users.name','payment.payment_ref','quotes.final_total','quotes.pending_amount','quotes.advance','searches.search_id')
            ...
            ->where('searches.user_id','=',$this->user_id)->get();
    ...
    public function MyPendingOrders(){
        ...
        $query = DB::table('searches')
            ...
            ->leftjoin('quotes','quotes.search_id','=','searches.search_id')->where('searches.url','!=',"")->where('searches.user_id','=',$this->user_id)->groupBy('searches.search_id')->get();
        foreach ($query as $value) {
            $containers = json_decode($value->containers);
            $routes = json_decode($value->routes); $info=array();
    ...
    public function trackQuote(Request $request){
        ...
        $data = DB::table('searches')
            ->select('searches.url')->where('searches.url','!=',"")
            ->where('searches.user_id','=',$this->user_id)->where('searches.search_id','=',$post['quote'])
            ->groupBy('searches.search_id')->first();
        //dd($data);
        if(@$data){
